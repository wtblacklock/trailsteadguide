/**
 * Per-guide gear assignments - which product fills which slot on each
 * `/guides/<slug>/` page. Sourced from `data/affiliate-coverage.csv`
 * (the curation worksheet) and consumed by `getGearForGuide()` in
 * `lib/affiliate/guide-gear.ts`.
 *
 * Guides not present in this map render no gear shelf. Each entry is
 * resolved against `AFFILIATE_PRODUCTS` at render time - invalid product
 * ids throw at build via `getProductById()`.
 */

import type { GearSlotId } from '@/lib/affiliate/gear-slots'

export type GuideGearEntry = {
  slot: GearSlotId
  productId: string
}

export const GUIDE_GEAR: Record<string, GuideGearEntry[]> = {
  'backyard-camping-with-kids': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
  ],
  'best-camping-sleeping-bag-for-kids': [
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
  ],
  'best-family-tent-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
  ],
  'best-tent-for-hot-weather': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'CANOPY', productId: 'core-10x10-canopy' },
  ],
  'best-tent-for-rainy-camping': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'tnf-wawona-6' },
  ],
  'campfire-recipes-for-kids': [
    { slot: 'KID_GEAR', productId: 'carpathen-smores-sticks' },
  ],
  'camping-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'best-state-parks-for-families': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
  ],
  'camping-in-a-heatwave': [
    { slot: 'TENT', productId: 'tnf-wawona-6' },
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-1-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'gci-freestyle-rocker' },
    { slot: 'HOT_GEAR', productId: 'frizcol-camping-fan' },
    { slot: 'CANOPY', productId: 'core-10x10-canopy' },
    { slot: 'SAFETY', productId: 'dripdrop-hydration' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'camping-in-california-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'camping-in-colorado-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_BAG', productId: 'marmot-mad-river-0' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'SLEEP_SURFACE', productId: 'rab-ionosphere-5-5' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'WINTER_GEAR', productId: 'hothands-hand-warmers-bulk' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'camping-in-florida-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'RAIN_GEAR', productId: 'geertop-17x10-tarp' },
    { slot: 'HOT_GEAR', productId: 'frizcol-camping-fan' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'dripdrop-hydration' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'camping-in-georgia-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'CANOPY', productId: 'core-10x10-canopy' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'HOT_GEAR', productId: 'frizcol-camping-fan' },
    { slot: 'SAFETY', productId: 'dripdrop-hydration' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'camping-in-michigan-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'RAIN_GEAR', productId: 'geertop-17x10-tarp' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'camping-in-new-york-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'RAIN_GEAR', productId: 'geertop-17x10-tarp' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'WINTER_GEAR', productId: 'hothands-hand-warmers-bulk' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'camping-in-pennsylvania-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'RAIN_GEAR', productId: 'geertop-17x10-tarp' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'SAFETY', productId: 'sawyer-extractor-pump-kit' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'camping-in-arizona-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'CANOPY', productId: 'core-10x10-canopy' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'HOT_GEAR', productId: 'frizcol-camping-fan' },
    { slot: 'SAFETY', productId: 'dripdrop-hydration' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'SAFETY', productId: 'sawyer-extractor-pump-kit' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'camping-in-montana-for-beginners': [
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'SLEEP_BAG', productId: 'marmot-mad-river-0' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'rab-ionosphere-5-5' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'WINTER_GEAR', productId: 'hothands-hand-warmers-bulk' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'camping-in-utah-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'CANOPY', productId: 'core-10x10-canopy' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'HOT_GEAR', productId: 'frizcol-camping-fan' },
    { slot: 'SAFETY', productId: 'dripdrop-hydration' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'camping-in-wisconsin-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'RAIN_GEAR', productId: 'geertop-17x10-tarp' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'SAFETY', productId: 'sawyer-extractor-pump-kit' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'camping-in-wyoming-for-beginners': [
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'SLEEP_BAG', productId: 'marmot-mad-river-0' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'rab-ionosphere-5-5' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'WINTER_GEAR', productId: 'hothands-hand-warmers-bulk' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'camping-in-texas-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'HOT_GEAR', productId: 'frizcol-camping-fan' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'dripdrop-hydration' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'camping-in-the-appalachians-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_BAG', productId: 'marmot-mad-river-0' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'SLEEP_SURFACE', productId: 'rab-ionosphere-5-5' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'WINTER_GEAR', productId: 'hothands-hand-warmers-bulk' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'camping-in-the-desert-southwest-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'HOT_GEAR', productId: 'frizcol-camping-fan' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'dripdrop-hydration' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'camping-in-the-northeast-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_BAG', productId: 'marmot-mad-river-0' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'SLEEP_SURFACE', productId: 'rab-ionosphere-5-5' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'WINTER_GEAR', productId: 'hothands-hand-warmers-bulk' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'camping-in-the-pacific-northwest-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_BAG', productId: 'marmot-mad-river-0' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'SLEEP_SURFACE', productId: 'rab-ionosphere-5-5' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'RAIN_GEAR', productId: 'geertop-17x10-tarp' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'camping-meal-plan-family': [
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
  ],
  'camping-when-the-weather-turns': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_BAG', productId: 'marmot-mad-river-0' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'SLEEP_SURFACE', productId: 'rab-ionosphere-5-5' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'RAIN_GEAR', productId: 'geertop-17x10-tarp' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'camping-with-dogs-first-time': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'DOG_GEAR', productId: 'katolk-tie-out' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'camping-with-kids-first-time': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'summer-camping-with-kids': [
    { slot: 'HOT_GEAR', productId: 'frizcol-camping-fan' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'aveeno-kids-sunscreen-lotion' },
    { slot: 'SAFETY', productId: 'dripdrop-hydration' },
    { slot: 'CANOPY', productId: 'core-10x10-canopy' },
  ],
  'camping-with-toddlers': [
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'KID_GEAR', productId: 'graco-pack-n-play-on-the-go' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SAFETY', productId: 'aveeno-kids-sunscreen-lotion' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'car-camping-beginner-guide': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'labor-day-weekend-camping': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
  ],
  'halloween-camping-with-kids': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'LIGHTING', productId: 'everbrite-headlamp-5-pack' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'WINTER_GEAR', productId: 'hothands-hand-warmers-bulk' },
    { slot: 'KID_GEAR', productId: 'glow-stick-necklaces-bulk' },
    { slot: 'KID_GEAR', productId: 'kids-craft-supply-kit' },
    { slot: 'KID_GEAR', productId: 'carpathen-smores-sticks' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
  ],
  'dispersed-camping-on-blm-and-national-forest-land': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'easy-family-camping-meals': [
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
  ],
  'fall-camping-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_BAG', productId: 'marmot-mad-river-0' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'SLEEP_SURFACE', productId: 'rab-ionosphere-5-5' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'WINTER_GEAR', productId: 'hothands-hand-warmers-bulk' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'fall-foliage-camping-with-kids': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'everbrite-headlamp-5-pack' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'WINTER_GEAR', productId: 'hothands-hand-warmers-bulk' },
    { slot: 'KID_GEAR', productId: 'peterson-first-guide-trees' },
    { slot: 'KID_GEAR', productId: 'celestron-outland-binoculars' },
    { slot: 'KID_GEAR', productId: 'nature-journal-art-kit' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
  ],
  'family-camping-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'family-camping-gear-list': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'family-camping-on-a-budget': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'STOVE', productId: 'coleman-1-burner' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
  ],
  'first-camping-trip-checklist': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'first-night-camping-guide': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'first-time-camping-mistakes': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'how-to-choose-a-family-campsite': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'CANOPY', productId: 'core-10x10-canopy' },
  ],
  'how-to-break-camp': [
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
    { slot: 'RAIN_GEAR', productId: 'geertop-17x10-tarp' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
  ],
  'how-to-pack-a-cooler': [
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
  ],
  'how-to-plan-a-camping-trip': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'how-to-set-up-a-tent': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
  ],
  'how-to-start-a-campfire': [
    { slot: 'SAFETY', productId: 'uco-stormproof-matches' },
    { slot: 'SAFETY', productId: 'esbit-fire-cubes' },
  ],
  'no-cook-camping-meals-kids': [
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
  ],
  'rainy-camping-trips': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'RAIN_GEAR', productId: 'geertop-17x10-tarp' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'recreation-gov-reservation-strategy': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
  ],
  'spring-camping-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_BAG', productId: 'marmot-mad-river-0' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'SLEEP_SURFACE', productId: 'rab-ionosphere-5-5' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'RAIN_GEAR', productId: 'geertop-17x10-tarp' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'summer-camping-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'HOT_GEAR', productId: 'frizcol-camping-fan' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'weekend-camping-packing-list': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
  'winter-camping-for-beginners': [
    { slot: 'TENT', productId: 'coleman-sundome-4p' },
    { slot: 'TENT', productId: 'core-6p-instant-cabin' },
    { slot: 'TENT', productId: 'alps-lynx-4p' },
    { slot: 'SLEEP_BAG', productId: 'coleman-brazos-bag' },
    { slot: 'SLEEP_BAG', productId: 'vumos-bag-liner' },
    { slot: 'SLEEP_BAG', productId: 'marmot-mad-river-0' },
    { slot: 'SLEEP_SURFACE', productId: 'big-agnes-divide' },
    { slot: 'SLEEP_SURFACE', productId: 'mondoking-3d-pad' },
    { slot: 'SLEEP_SURFACE', productId: 'lost-horizon-air-foam-mattress' },
    { slot: 'SLEEP_SURFACE', productId: 'rab-ionosphere-5-5' },
    { slot: 'STOVE', productId: 'coleman-triton-2-burner' },
    { slot: 'COOKWARE', productId: 'thtybros-cookware-mess-kit' },
    { slot: 'COOLER', productId: 'coleman-classic-rolling-cooler' },
    { slot: 'LIGHTING', productId: 'luminaid-packlite-max' },
    { slot: 'LIGHTING', productId: 'streamlight-protac-2' },
    { slot: 'LIGHTING', productId: 'black-diamond-spot-400' },
    { slot: 'CHAIR', productId: 'coleman-portable-chair-cooler' },
    { slot: 'WINTER_GEAR', productId: 'hothands-hand-warmers-bulk' },
    { slot: 'KID_GEAR', productId: 'kidco-gopod' },
    { slot: 'SAFETY', productId: 'thriad-first-aid-430' },
    { slot: 'POWER', productId: 'anker-zolo-power-bank' },
    { slot: 'TRASH', productId: 'fwc-trash-can-wakeman' },
  ],
}
