...
export default function GearHubClient({ bundles }: Props) {
  const [tierByCard, setTierByCard] = useState<Record<PlanSlug, GearTier>>(() =>
    Object.fromEntries(bundles.map((b) => [b.planSlug, 'standard'])) as Record<PlanSlug, GearTier>,
  )
  ...
  return (
    <section className="max-w-page mx-auto px-8 pb-24">
      <ul className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        {bundles.map((bundle) => {
          const tier = tierByCard[bundle.planSlug]
          const items = bundle.itemsByTier[tier]
          const total = bundle.totalByTier[tier]
          return (
            <li key={bundle.planSlug} className="rounded-2xl ring-1 ring-stone-200 bg-white overflow-hidden">
              <div className="p-8 md:p-10 pb-2">
                ...
                <GearTierToggle
                  tier={tier}
                  onTierChange={(next) => setTierByCard((prev) => ({ ...prev, [bundle.planSlug]: next }))}
                />
              </div>
